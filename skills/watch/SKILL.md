...
test -f "$SKILL_DIR/scripts/watch.py" || echo "ERROR: wrong SKILL_DIR" >&2
```

## Step 0 — setup preflight (first `/watch` call in a session)
...
- `first_run: true` → run the installer, which installs missing binaries
  ...
  scaffolds `~/.config/watch-skill/.env`:
  ```bash
  ...
  block on it. Without a key, `/watch` still works — videos without
...
Set via `WATCH_DETAIL` in `~/.config/watch-skill/.env`, or `--detail` per
call. Default: `balanced`.

| Detail | Frames | Engine |
|---|---|---|
| `transcript` | none | skips video download when captions exist |
| `efficient` | up to 50 | keyframes only — near-instant |
| `balanced` (default) | up to 100 | scene-change detection |
| `token-burner` | uncapped | scene-change detection, every shot kept |

All tiers cap at 2 fps regardless of budget. A video with too few distinct
shots for scene/keyframe detection to be meaningful (a static talking-head
recording, for instance) falls back to uniform time-based sampling
automatically.
...
3. Even at `transcript` detail with no frames, produce a synthesized
...
- Reads/creates `~/.config/watch-skill/.env` (mode `0600`) for API keys and
  a setup-complete marker
...
**Bundled scripts:** `scripts/watch.py` (entry point), `scripts/download.py`
(yt-dlp wrapper), `scripts/frames.py` (ffmpeg frame extraction + dedup),
`scripts/transcribe.py` (caption parsing), `scripts/whisper.py` (Groq/OpenAI
clients), `scripts/setup.py` (preflight + installer), `scripts/config.py`

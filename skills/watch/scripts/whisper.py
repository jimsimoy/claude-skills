...
def load_api_key(prefer: str | None = None) -> tuple[str, str] | tuple[None, None]:
    ...
    dotenv_candidates = [Path.home() / ".config" / "watch-skill" / ".env", Path.cwd() / ".env"]
...
def _post(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "watch-skill/1.0 (+claude-code)",
    }
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if attempt < MAX_ATTEMPTS - 1:
                ...
                print(f"[watch] whisper network error — retrying in {delay:.1f}s", file=sys.stderr)
                time.sleep(delay)
    ...
    if audio_bytes <= MAX_UPLOAD_BYTES:
        ...
        for i, (path, offset) in enumerate(chunks):
            try:
                ...
                failures += 1
                print(f"[watch] chunk {i + 1}/{len(chunks)} failed: {exc}", file=sys.stderr)

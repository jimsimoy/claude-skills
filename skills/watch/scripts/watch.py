...
from config import frame_cap, get_detail  # noqa: E402
...
from transcribe import filter_range, parse_vtt, render as render_transcript  # noqa: E402
from whisper import load_api_key, transcribe_video  # noqa: E402

_ENGINE_FOR_DETAIL = {"efficient": "keyframe", "balanced": "scene", "token-burner": "scene"}


def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(prog="watch", description="Watch a video: frames + transcript.")
    ap.add_argument("source", help="Video URL or local file path")
    ap.add_argument("--detail", choices=["transcript", "efficient", "balanced", "token-burner"], default=None)
    ...
    ap.add_argument("--max-frames", type=int, default=None, help="Override the detail tier's frame cap")
...
def main() -> int:
    ...
    detail = args.detail or get_detail()
    ...
    if url_source:
        ...
        print("[watch] checking for captions…", file=sys.stderr)
        dl = fetch_captions_only(args.source, work / "download")
        if dl.get("subtitle_path"):
            transcript_segments = parse_vtt(dl["subtitle_path"])
            transcript_source = "captions"

    need_video = not (detail == "transcript" and transcript_segments)
    if need_video:
        audio_only = detail == "transcript"
        print("[watch] downloading…" if url_source else "[watch] reading local file…", file=sys.stderr)
    ...
    if detail != "transcript" and video_path:
        ...
        engine = _ENGINE_FOR_DETAIL[detail]
        print(f"[watch] extracting frames ({engine})…", file=sys.stderr)
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and api_key:
            ...
            print(f"[watch] no captions and no Whisper key — run {SCRIPT_DIR / 'setup.py'}", file=sys.stderr)

    _print_report(args, detail, full_duration, focused, eff_start, eff_end, eff_duration, meta, dl, cap, frames, frame_info, transcript_segments, transcript_source, work)
    return 0


def _print_report(args, detail, full_duration, focused, eff_start, eff_end, eff_duration, meta, dl, cap, frames, frame_info, transcript_segments, transcript_source, work) -> None:
    info = dl.get("info") or {}
    print()
    ...
    if not focused and full_duration > 600 and detail not in ("transcript", "token-burner"):
        ...
        print(
            ...
            "length under this detail tier. Re-run with --start/--end to focus on a section, or "
            "--detail token-burner for full scene coverage."
        )
